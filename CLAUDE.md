...
- 2026-07-13 marathon: PANEL armed + first live run ($1.24, FAIL against the
  ...
  (streak day 1 of 7).
- M6b BUILT, AWAITING OWNER ACCEPTANCE (2026-07-13 evening). Panel consult
  ran ($1.54, ADOPT-WITH-CHANGES): linear detrending REJECTED; the panel-
  synthesized replacement (quadratic residualization, identity-inclusive
  reprojected permutation, studentized max-|z|, plus-one p, 1999 reps,
  generator overhaul + shared-curvature stress null) is recorded verbatim
  in docs/specs/m6b_panel_amendment2.md. Implemented in
  analysis/{phase1_detectability,synthetic}.py; tests RESTORED to
  tests/analysis/ (wip/ deleted); full pytest gate GREEN. OUTCOME:
  amendment2's own exhaustion clause was REACHED — attempt 1 (circular
  shift) failed the shared-curvature null; attempt 2 (stratified swap)
  calibrated for LOWER-tier (clean, all 3 seeds) but not TOP-tier (2/3
  seeds false-positive) → CONFIRMATORY_INFERENCE_RETIRED=True; the
  deliverable is a null-calibrated DESCRIPTIVE analysis, no significant-
  bin claims. Evidence independently reproduced by the lead:
  docs/reports/m6b_shared_curvature_null_validation.md (deterministic,
  regenerated by scripts/m6b_null_validation.py). ACCEPTANCE CAVEAT:
  reviewer gate NOT run — the Agent dispatch layer hangs in the desktop-
  app client (dispatched agents complete their file work but their stop
  signals wedge; two implementer dispatches surfaced as interrupted/
  rejected while their writes landed). Lead verified via gate + the
  independent validation reproduction instead. Owner may request a
  reviewer pass from a CLI session before accepting.
